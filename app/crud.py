...
async def create_order(db: AsyncSession, order_data: schemas.OrderCreate, user_id: int):
    ...
    try:
        ...
        if final_amount < 0:
            raise HTTPException(
                status_code=400,
                detail="Chiết khấu không được lớn hơn tổng tiền sau thuế.",
            )
        ...
        await db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Lỗi hệ thống khi tạo đơn hàng: {str(e)}"
        )
...
async def restock_inventory(
    db: AsyncSession, product_id: int, quantity: int, user_id: int
) -> models.Product:
    ...
    if not product:
        raise HTTPException(
            status_code=404, detail=f"Sản phẩm ID {product_id} không tồn tại."
        )
...
async def clear_all_invoices(db: AsyncSession):
    try:
        ...
        await db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Lỗi khi xóa lịch sử hóa đơn: {str(e)}"
        )
...
async def create_product(
    ...
    if not category:
        raise HTTPException(
            status_code=500,
            detail="Hệ thống chưa có danh mục mặc định. Vui lòng chạy lại file seed.py.",
        )
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Lỗi lưu CSDL thêm sản phẩm: {str(e)}"
        )
